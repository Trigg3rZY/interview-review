...
```js
// 想吐槽一下深拷贝问题，深拷贝其实应该是个比较复杂的问题，涉及到类型判断、类型构造、属性遍历、元编程等很多基础知识。面试的时候大多只是考察个基础类型和引用类型的情况，很多流传的八股文答案覆盖并不全面。但是面试官又经常不满足于候选人说出思路，苛求具体实现，但是一个背下来的覆盖情况并不全面的所谓“深拷贝函数”又有什么意义
// 还不如拆分成涉及到的各个方面的子问题单独提问呢，比如对类型判断、属性描述符等提问
